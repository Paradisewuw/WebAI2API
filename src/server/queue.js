...
export function createQueueManager(queueConfig, callbacks) {
    ...
    async function processTask(task) {
        const { res, prompt, imagePaths, modelId, modelName, id, isStreaming, reasoning, codexInstallationId } = task;
        ...
        try {
            ...
            // 调用核心生图逻辑 (通过 Pool 分发)
            const meta = { id, reasoning };
            if (codexInstallationId) {
                Object.defineProperty(meta, 'codexInstallationId', {
                    value: codexInstallationId,
                    enumerable: false
                });
            }
            const result = await generate(poolContext, prompt, imagePaths, modelId, meta);
